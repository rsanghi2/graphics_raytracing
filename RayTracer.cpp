...
int RayTracer::aaImage() {
  ...
  for (int i = 0; i < buffer_width; i ++) {
    for (int j = 0; j < buffer_height; j ++) {
      glm::dvec3 colorAvg = glm::dvec3{0,0,0};
      for (double sampleX = 0; sampleX < samplesPerSide; sampleX++) {
        for (double sampleY = 0; sampleY < samplesPerSide; sampleY++) {
          ...
          double dummy;
          colorAvg += traceRay(toCast, glm::dvec3 {1,1,1}, traceUI->getDepth(), dummy); // ?
        }
      }
      colorAvg = colorAvg / (double) samples;
      colorAvg = glm::clamp(colorAvg, 0.0, 1.0);
      setPixel(i, j, colorAvg);
    }
  }
